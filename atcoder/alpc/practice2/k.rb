...
a    = gets.to_s.split.map { |e| (e.to_i << 32) + 1 }
...
mapping = Proc.new{ |f, a|
  fb, fc = (f >> 32), (f % (1 << 32))
  av, an = (a >> 32), (a % (1 << 32))
  (((fb * av + fc * an) % MOD) << 32) + an
}
composition =  Proc.new{ |f, g|
  fb, fc = f >> 32, f % (1 << 32)
  gb, gc = g >> 32, g % (1 << 32)
  ((fb * gb % MOD) << 32) + ((gc * fb + fc) % MOD)
}

lst = LazySegtree.new(a, 0, (1 << 32), op, mapping, composition)
ans = []
q.times do
  query = gets.to_s.split.map{ |e| e.to_i }
  if query[0] == 0
    _, l, r, a, b = query
    lst.range_apply(l, r, (a << 32) + b)
  else
    _, l, r = query
    ans << (lst.prod(l, r) >> 32)
  end
