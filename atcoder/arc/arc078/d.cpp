...
void dfs(int start, int ds, int dist_flag, const vector<vector<int>> &graph){
    stack<pair<int, int>> st;
    st.push({start, ds});
    while(!st.empty()){
        pair<int, int> t = st.top(); st.pop();
        int from = t.first, d = t.second;
        for(auto &e : graph[from]){
            if(dist[dist_flag][e] > d + 1){
                dist[dist_flag][e] = d+1;
                st.push({e, d+1});
            }
        }
    }
...
int main() {
    ...
    // rep(i, N) cout << dist[0][i] << endl;
    // cout << "---" << endl;
    // rep(i, N) cout << dist[1][i] << endl;
